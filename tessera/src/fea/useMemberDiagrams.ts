...
import { computeMemberDiagrams, type MemberDiagram } from './feaDiagrams';
...
export function useMemberDiagrams(beam: MemberBeam): MemberDiagramsState {
  const [state, setState] = useState<MemberDiagramsState>({ status: 'idle', diagram: null });
  const { lengthIn, E, A, I, w } = beam;

  useEffect(() => {
    if (!valid(beam)) {
      setState({ status: 'idle', diagram: null });
      return;
    }
    let cancelled = false;
    setState((s) => (s.status === 'ready' ? s : { status: 'loading', diagram: null }));
    ...
    getEngine()
      ...
      .then((result) => {
        if (cancelled) return;
        const [diagram] = computeMemberDiagrams(normalizeFeaModel(input), result, { stations: 41 });
        setState({ status: 'ready', diagram: diagram ?? null });
      })
      .catch((e: unknown) => {
        if (!cancelled) {
          setState({ status: 'unavailable', diagram: null, error: e instanceof Error ? e.message : String(e) });
        }
      });
