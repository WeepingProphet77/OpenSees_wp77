 ...
 * out-of-plane). Yaw/pitch sliders orbit the camera; the view auto-fits.
 */
...
export function Member3DView({
  ...
  const view = useMemo(() => {
    ...
    // Section bounding box → centre the extrusion on its own centroid so the
    // yaw/pitch orbit is about the member's middle.
    ...
    // Section coords: x = horizontal width, y = depth from the top fibre (down).
    // World: x = span (I→J), y = vertical up, z = out-of-plane width.
    const toWorld = (p: Point, t: number): Vec3 => ({ x: t - L / 2, y: midY - p.y, z: p.x - midX });

    const project = projector(yawDeg * DEG, pitchDeg * DEG);
    // Screen y is up-positive; flip for SVG.
    const raw = (v: Vec3) => {
      const pr = project(v);
    ...
    // Auto-fit over every screen point we will draw.
    const fitPts = [
    ...
    // Painter ordering: the end face with the greater average depth is nearer the
    // camera → draw it last (filled, on top); draw the far face first as outline.
  ...
  }, [section, lengthIn, layers, size, yawDeg, pitchDeg]);
      ...
      <svg viewBox={`0 0 ${view.W} ${view.H}`} width="100%" style={{ maxWidth: view.W }} role="img" aria-label="Member 3D view">
        ...
        {view.reinf.map((r, i) => (
          <line
            ...
            x2={r.x2}
            y2={r.y2}
            stroke={r.strand ? '#f59e0b' : 'var(--foreground)'}
            strokeWidth={2}
            strokeOpacity={0.85}
            strokeLinecap="round"
          />
